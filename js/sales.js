'use strict';
import {jq} from './fakeQuery.js';
 (function (){
  let $ = jq();
  //console.log($);
  //     Duplicate code has been removed and DRY principles are evident
  let table = $.elem.getElemSelector('table');
  let locationArray = [];
  let newStoreElement = $.elem.getElemId('salesForm');
  ...
  let getSalesNumbers = function (maxCust, minCust, avgCookieSale) {
    let total = 0;
    let temp = 0;
    let res = {
      totalSold: 0,
  ...
  let calculateDaysSales = function (locationArray) {
  ...
  //Store Constructor
  let Store = function (locationName, minCust, maxCust, avgCookieSale, cookiesSold = {}) {
  ...
  let makeTimeString = function (hour) {
    let amPm = 'am';
    if (hour > 12) {
      hour -= 12;
      amPm = 'pm';
  ...
  let drawTable = function (tableEl, locationArray) {
    for (let i = 0; i < locationArray.length; i++) {
      let workingRow =  $.elem.newElem('tr');
      //location name
      let locationNameEl =  $.elem.newElem('td');
  ...
  let drawTableHead = function () {
    let headEl =  $.elem.newElem('thead');
    let tdEl = $.elem.newElem('td');
  ...
  let drawTableFooter = function () {
    let hourlyTotals = []; 
    let tfootEl = $.elem.newElem('tfoot');
    let tdEl = document.createElement('td'); $.elem.newElem('td');
    ...
    let totalTotals = $.elem.newElem('td');
